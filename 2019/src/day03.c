...
static size_t cell_hash(struct cell* element);
static size_t cell_equals(struct cell* a, struct cell* b);
...
size_t cell_hash(cell* element)
{
	return vec2_hash(element->key);
}

size_t cell_equals(cell* a,  cell* b)
{
