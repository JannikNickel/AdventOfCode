...
namespace AdventOfCode2022.Day22
{
    public class Solution : SolutionBase
    {
        ...
        public override object? SolveFirst()
        {
            ...
            return CalcMapScore(map, path, pos);
        }
        ...
        private static int CalcMapScore(Map map, List<Step> path, Vec2 pos)
        {
        ...
        private static int CalcScore(Vec2 player, Vec2 dir)
        {
            int facing = dir == new Vec2(1, 0) ? 0 : (dir == new Vec2(0, -1) ? 1 : (dir == new Vec2(-1, 0) ? 2 : 3));
            return 1000 * (player.y + 1) + 4 * (player.x + 1) + facing;
        }

        private static char[,] ParseMap(string input, out List<Step> path, out Vec2 startPos)
        {
            ...
            while(index < pathInput.Length)
            {
                if(char.IsDigit(pathInput[index]))
                {
                    ...
                    while(++index < pathInput.Length && char.IsDigit(pathInput[index]))
                        continue;
                    path.Add(new Step(int.Parse(pathInput[from..index])));
                    continue;
                }
        ...
        {
            private readonly Dictionary<(Vec2 from, Vec2 dir), (Vec2 target, Vec2 dir)> jumps;
