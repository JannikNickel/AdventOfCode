...
namespace AdventOfCode2022.Day13
{
    public class Solution : SolutionBase
    {
        ...
        private static List<(NestedList left, NestedList right)> ParsePairs(string input)
        {
        ...
        {
            private readonly List<NestedList> elements = new List<NestedList>();

            public int Value { get; init; } = -1;
            public bool IsValue => Value != -1;
            public int Count => elements.Count;

            public NestedList this[int index] => elements[index];
            ...
            public static NestedList Parse(string line)
            {
                ...
                for(int i = 1;i < line.Length - 1;i++)
                {
                    if(char.IsDigit(line[i]))
                    {
                        ...
                        int value = int.Parse(line[from..i]);
                        list.elements.Add(new NestedList(value));
                    }
                    else if(line[i] == '[')
                    {
                        ...
                        while(open != 0);
                        i++;
                        list.elements.Add(Parse(line[from..i]));
                    }
                }
                return list;
            }
