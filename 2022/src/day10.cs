...
namespace AdventOfCode2022.Day10
{
    public class Solution : SolutionBase
    {
        ...
        private static void SimulateCPU(string[] instructions, Action<int, int> tick)
        {
            int xReg = 1;
        ...
        private static IEnumerator<(OpCode op, int param)> LoadInstructions(string[] lines)
        {
            foreach((OpCode op, int param) instruction in lines.Select(n => ParseInstruction(n)))
            {
                yield return instruction;
            }
        }

        private static IEnumerator<(bool completed, int result)> ExecuteInstruction(OpCode instruction, int param, int xReg)
        {
            switch(instruction)
            {
                case OpCode.noop:
        ...
        private static (OpCode op, int param) ParseInstruction(string line)
        {
