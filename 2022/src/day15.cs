...
namespace AdventOfCode2022.Day15
{
    ...
    {
        private static readonly Vec2[] diamondDirs = new Vec2[] { new Vec2(1, -1), new Vec2(1, 1), new Vec2(-1, 1), new Vec2(-1, -1) };
        ...
        private static int CountBlockedInRow((Vec2 sensor, Vec2 beacon)[] sensors, int row)
        {
        ...
        private static int ManhattenDst(Vec2 from, Vec2 to)
        {
            return Math.Abs(to.x - from.x) + Math.Abs(to.y - from.y);
        }

        private static (Vec2 sensor, Vec2 beacon)[] ParseSensorData(string[] lines)
        {
            return lines
